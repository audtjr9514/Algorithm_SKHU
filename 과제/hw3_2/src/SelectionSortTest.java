/* ************************
파일명 : SelectionSortTest.java
작성자 : 김명석
작성일 : 20.04.03
내용 : 학생 수를 입력 받고, 각 학생의 정보들을 클래스 배열로 저장한 뒤,
       전체 학생의 수를 선택 정렬을 이용하여 오름차순으로 만든 뒤, 출력하는 프로그램
 ************************ */
...
public class SelectionSortTest {
    public static void main(String[] args) throws IOException {
        ...
        bw.write("학생수 입력 : ");
        bw.flush();
        int N = Integer.parseInt(br.readLine()); // 사용자가 입력하는 학생 정보의 수
        ...
        for (int i = 0; i < N - 1; i++) {
            min = i;
            for (int j = i + 1; j < N; j++) {
                if (Students[j].getName().compareTo(Students[min].getName()) <= 0)
                    // String의 compareTo를 이용하여 String의 사전순 크기를 비교
                    min = j;
            }
            temp = Students[min];
